...
type EditProductOverlayProps = {
  visible: boolean;
  product: StoreOwnerProduct | null;
  onClose: () => void;
  onSubmit: (productId: string, product: Partial<StoreOwnerProduct>, imageUri?: string) => void;
  categories: string[];
};
...
export function EditProductOverlay({
  ...
  useEffect(() => {
    if (product && visible) {
      ...
      setImageUri(null); // Reset new image selection
    }
  }, [product, visible]);
  ...
  const handleSubmit = () => {
    ...
    const updatedProduct: Partial<StoreOwnerProduct> = {
      name: name.trim(),
    ...
    // Pass imageUri if a new image was selected, otherwise pass undefined to keep existing
    const imageToUpdate = imageUri || undefined;

    onSubmit(product.id, updatedProduct, imageToUpdate);
  };
